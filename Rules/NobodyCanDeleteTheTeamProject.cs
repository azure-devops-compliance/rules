...
namespace AzureDevOps.Compliance.Rules
{
    public class NobodyCanDeleteTheTeamProject : IProjectRule, IProjectReconcile
    {
        ...
        [ExcludeFromCodeCoverage] public string Link => "https://confluence.dev.rabobank.nl/x/NY8AD";
        ...
        private async Task<bool> CheckProjectAdministratorsGroupOnlyContainsRabobankAdministratorsAsync(string project)
        {
            var groups = await _client
                .GetAsync(SecurePipelineScan.VstsService.Requests.ApplicationGroup.ApplicationGroups(project))
                .ConfigureAwait(false);
            ...
            var members = (await _client
                    .GetAsync(SecurePipelineScan.VstsService.Requests.ApplicationGroup.GroupMembers(project, id))
                    .ConfigureAwait(false))
                .Identities;
        ...
        {
            var groups = await _client.GetAsync(SecurePipelineScan.VstsService.Requests.ApplicationGroup.ApplicationGroups(projectId)).ConfigureAwait(false);
            ...
            var members = (await _client.GetAsync(SecurePipelineScan.VstsService.Requests.ApplicationGroup.GroupMembers(projectId, paId)).ConfigureAwait(false))
                .Identities
        ...
        private async Task<ApplicationGroup> CreateRabobankProjectAdministratorsGroupsIfNotExistsAsync(string project, Response.ApplicationGroups groups) =>
            ...
            ?? await _client
                .PostAsync(SecurePipelineScan.VstsService.Requests.Security.ManageGroup(project), new SecurePipelineScan.VstsService.Requests.Security.ManageGroupData { Name = RabobankProjectAdministrators })
                .ConfigureAwait(false);
        ...
        private async Task AddAllMembersToRabobankProjectAdministratorsGroupAsync(string project, IEnumerable<ApplicationGroup> members, string rabo) =>
            await _client.PostAsync(SecurePipelineScan.VstsService.Requests.Security.AddMember(project),
                    new SecurePipelineScan.VstsService.Requests.Security.AddMemberData(
        ...
        private async Task RemoveAllOtherMembersFromProjectAdministratorsAsync(string project, IEnumerable<ApplicationGroup> members, string id) =>
            await _client.PostAsync(SecurePipelineScan.VstsService.Requests.Security.EditMembership(project),
                    new SecurePipelineScan.VstsService.Requests.Security.RemoveMembersData(members.Select(m => m.TeamFoundationId), id))
                .ConfigureAwait(false);
        ...
        private async Task AddRabobankProjectAdministratorsToProjectAdministratorsGroupAsync(string project, string rabo, string id) =>
            await _client.PostAsync(SecurePipelineScan.VstsService.Requests.Security.AddMember(project),
                    new SecurePipelineScan.VstsService.Requests.Security.AddMemberData(new[] { rabo }, new[] { id }))
                .ConfigureAwait(false);
    }
}